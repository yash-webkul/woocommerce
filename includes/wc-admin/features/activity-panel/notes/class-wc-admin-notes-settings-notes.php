...
class WC_Admin_Notes_Settings_Notes {
	...
	protected static function possibly_add_notice_setting_moved_note() {
		...
		$note->set_title( __( 'Looking for the Store Notice setting?', 'woocommerce' ) );
		$note->set_content( __( 'It can now be found in the Customizer.', 'woocommerce' ) );
		...
		$note->add_action(
			'open-customizer',
			__( 'Open Customizer', 'woocommerce' ),
			'customize.php'
		);
		$note->save();
	}
