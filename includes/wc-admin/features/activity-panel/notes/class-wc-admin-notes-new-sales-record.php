	...
	public static function possibly_add_sales_record_note() {
		...
		if ( $total > $record_amt ) {
			...
			$content = sprintf(
				...
				__( 'Woohoo, %1$s was your record day for sales! Net revenue was %2$s beating the previous record of %3$s set on %4$s.', 'woocommerce' ),
				$formatted_yesterday,
				$formatted_total,
			...
			$note->add_action( 'view-report', __( 'View report', 'woocommerce' ), '?page=wc-admin#/analytics' );
			$note->save();
		}
	}
}
