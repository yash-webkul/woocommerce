...
if ( ! function_exists( 'wc_admin_register_page' ) ) {
	...
	function wc_admin_register_page( $options ) {
		...
		add_submenu_page(
			...
			array( 'WC_Admin_Library', 'page_wrapper' )
		);
	}
}

/**
 * WC_Admin_Library Class.
 */
class WC_Admin_Library {

	/**
	 * Class instance.
	 *
	 * @var WC_Admin_Library instance
	 */
	...
	public function __construct() {
		add_action( 'init', array( 'WC_Admin_Library', 'load_features' ) );
		add_action( 'admin_enqueue_scripts', array( 'WC_Admin_Library', 'register_scripts' ) );
		add_action( 'admin_enqueue_scripts', array( 'WC_Admin_Library', 'load_scripts' ), 15 );
		add_action( 'woocommerce_components_settings', array( 'WC_Admin_Library', 'add_component_settings' ) );
		add_filter( 'admin_body_class', array( 'WC_Admin_Library', 'add_admin_body_classes' ) );
		add_action( 'admin_menu', array( 'WC_Admin_Library', 'register_page_handler' ) );
		add_filter( 'admin_title', array( 'WC_Admin_Library', 'update_admin_title' ) );
		add_action( 'rest_api_init', array( 'WC_Admin_Library', 'register_user_data' ) );
		add_action( 'in_admin_header', array( 'WC_Admin_Library', 'embed_page_header' ) );
		add_filter( 'woocommerce_settings_groups', array( 'WC_Admin_Library', 'add_settings_group' ) );
		add_filter( 'woocommerce_settings-wc_admin', array( 'WC_Admin_Library', 'add_settings' ) );
		add_action( 'admin_head', array( 'WC_Admin_Library', 'remove_notices' ) );
		add_action( 'admin_notices', array( 'WC_Admin_Library', 'inject_before_notices' ) );
		add_action( 'admin_notices', array( 'WC_Admin_Library', 'inject_after_notices' ), PHP_INT_MAX );

		// priority is 20 to run after https://github.com/woocommerce/woocommerce/blob/a55ae325306fc2179149ba9b97e66f32f84fdd9c/includes/admin/class-wc-admin-menus.php#L165.
		add_action( 'admin_head', array( 'WC_Admin_Library', 'update_link_structure' ), 20 );
	}
	...
	public static function get_file_version( $file ) {
		if ( defined( 'SCRIPT_DEBUG' ) && SCRIPT_DEBUG ) {
			$file = trim( $file, '/' );
			return filemtime( WC_ABSPATH . self::get_path( $file ) );
		}
		return WC_VERSION;
	}
	...
	private static function get_path( $file ) {
		return '.css' === substr( $file, -4 ) ? '/assets/css/wc-admin/' : '/assets/js/wc-admin/';
	}
	...
	public static function load_features() {
		...
		foreach ( $features as $feature ) {
			...
			$file    = WC_ABSPATH . 'includes/wc-admin/features/' . $feature . '/class-wc-admin-' . $feature . '.php';
			if ( file_exists( $file ) ) {
				require_once( $file );
	...
	public static function register_page_handler() {
		...
		if ( self::is_feature_enabled( 'dashboard' ) ) {
			$page_title = __( 'WooCommerce Dashboard', 'woocommerce' );
	...
	public static function register_scripts() {
		...
		wp_set_script_translations( 'woocommerce-admin', 'woocommerce' );
		...
		wp_style_add_data( 'woocommerce-admin', 'rtl', 'replace' );
	}
	...
	/**
	 * `WC_Admin_Library::get_embed_enabled_screen_ids`,  `WC_Admin_Library::get_embed_enabled_plugin_screen_ids`,
	 * `WC_Admin_Library::get_embed_enabled_screen_ids` should be considered temporary functions for the feature plugin.
	...
	private static function get_embed_breadcrumbs() {
		...
		$breadcrumbs = apply_filters(
			...
			array(
				'edit-shop_order'                       => __( 'Orders', 'woocommerce' ),
				'add-shop_order'                        => array(
					array( '/edit.php?post_type=shop_order', __( 'Orders', 'woocommerce' ) ),
					__( 'Add New', 'woocommerce' ),
				),
				'shop_order'                            => array(
					array( '/edit.php?post_type=shop_order', __( 'Orders', 'woocommerce' ) ),
					__( 'Edit Order', 'woocommerce' ),
				),
				'edit-shop_coupon'                      => __( 'Coupons', 'woocommerce' ),
				'add-shop_coupon'                       => array(
					array( 'edit.php?post_type=shop_coupon', __( 'Coupons', 'woocommerce' ) ),
					__( 'Add New', 'woocommerce' ),
				),
				'shop_coupon'                           => array(
					array( 'edit.php?post_type=shop_coupon', __( 'Coupons', 'woocommerce' ) ),
					__( 'Edit Coupon', 'woocommerce' ),
				),
				'woocommerce_page_wc-reports'           => array(
					array( 'admin.php?page=wc-reports', __( 'Reports', 'woocommerce' ) ),
				),
				...
				'woocommerce_page_wc-settings'          => array(
					array( 'admin.php?page=wc-settings', __( 'Settings', 'woocommerce' ) ),
				),
				...
				'status-woocommerce_page_wc-status'     => array(
					array( 'admin.php?page=wc-status', __( 'Status', 'woocommerce' ) ),
				...
				'connect-woocommerce_page_wc-status'    => array(
					array( 'admin.php?page=wc-status', __( 'Status', 'woocommerce' ) ),
					__( 'WooCommerce Services Status', 'woocommerce' ),
				),
				'woocommerce_page_wc-addons'            => __( 'Extensions', 'woocommerce' ),
				'edit-product'                          => __( 'Products', 'woocommerce' ),
				'product_page_product_importer'         => array(
					array( 'edit.php?post_type=product', __( 'Products', 'woocommerce' ) ),
	...
	public static function update_admin_title( $admin_title ) {
		...
		return sprintf( __( '%1$s &lsaquo; %2$s &#8212; WooCommerce', 'woocommerce' ), $title, get_bloginfo( 'name' ) );
	}
	...
	public static function register_user_data() {
		register_rest_field(
			...
			'woocommerce_meta',
			array(
				'get_callback'    => array( 'WC_Admin_Library', 'get_user_data_values' ),
				'update_callback' => array( 'WC_Admin_Library', 'update_user_data_values' ),
				'schema'          => null,
			)
		);
	}
	...
	/**
	 * For all the registered user data fields (  WC_Admin_Library::get_user_data_fields ), fetch the data
	...
	/**
	 * For all the registered user data fields ( WC_Admin_Library::get_user_data_fields ), update the data
	 * for the REST API.
	 *
